...
def test_daily_sync_writes_raw_and_core(tmp_path: Path):
    if not _docker_available():
        pytest.skip("docker not available")

    # Start postgres
    container = "api-football-it-postgres-daily"
    ...
    try:
        ...
        fixtures_full = json.loads((fixtures_dir / "fixtures_full.json").read_text())
        ...
        # Run daily sync with mocked API client
        import asyncio

        fake = _FakeClient(fixtures_full)
        summary = asyncio.run(
            sync_daily_fixtures(
        ...
        assert fake.calls == 1
        assert query_scalar("SELECT COUNT(*) FROM raw.api_responses WHERE endpoint='/fixtures'") == 1
        assert query_scalar("SELECT COUNT(*) FROM core.fixtures") == 1
        assert query_scalar("SELECT COUNT(*) FROM core.fixture_details") == 1
    finally:
        subprocess.run(["docker", "rm", "-f", container], check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
