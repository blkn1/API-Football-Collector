...
async def get_fixture_detail_status(fixture_id: int) -> dict:
    ...
    try:
        ...
        fx = mcp_schemas.FixtureDetailStatus(
            ...
            status_short=row[4],
            needs_score_verification=bool(row[5]) if row[5] is not None else None,
            verification_state=(str(row[6]) if row[6] is not None else None),
            verification_attempt_count=_to_int_or_none(row[7]),
            verification_last_attempt_at_utc=_to_iso_or_none(row[8]),
            has_players=bool(row[9]),
            has_events=bool(row[10]),
            has_statistics=bool(row[11]),
            has_lineups=bool(row[12]),
            last_players_fetch_utc=_to_iso_or_none(row[13]),
            last_events_fetch_utc=_to_iso_or_none(row[14]),
            last_statistics_fetch_utc=_to_iso_or_none(row[15]),
            last_lineups_fetch_utc=_to_iso_or_none(row[16]),
        )
